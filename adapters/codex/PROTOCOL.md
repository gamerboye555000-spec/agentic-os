...
1. **Read the context pack first.** It lives at
   `.agentic-os/packs/T-XXXX-codex.md` and carries the goal, acceptance
...
5. **Write back, then end the run:**
   ...
   ```
   python aos.py evidence add T-XXXX --kind test --ref "<what proves it>" --claim "<what it proves>" --provenance agent:codex
...
Create a dropfile at `.agentic-os/exports/dropfile-<T-XXXX>-codex-<n>.md`
