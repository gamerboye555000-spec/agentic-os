...
class TestDoctorSecretSweep(WarnCase):
    ...
    def test_clean_workspace_sweep_passes(self):
        ...
        # 18 → 20 → 21 → 25 → 30 → 31 → 34 → 37: the two U-H2 warn-only
        ...
        # checks, then U-A2's three built-in catalog checks joined the
        # mandated set (D-W8.1 pattern — the pin moves UP with mandated new
        # checks; this fixture never installs the catalog, so all three
        # stay [PASS]).
        self.assertEqual(
            len([l for l in out.strip().splitlines() if l]), 37
        )
