...
class TestDoctorHardening(Night1BackCompatCase):
    ...
    def test_clean_weekend_workspace_passes_all_checks(self):
        ...
        # 20 → 21 → 25 → 30 → 31 → 34 → 37: the mandated U-E2 runtime power
        # state check joined the set, then U-M2's four memory-claim checks,
        # then U-M3's five memory-graph checks, then U-M5's retrieval
        # benchmark registry check, then U-A1's three agent-registry
        # checks, then U-A2's three built-in catalog checks (D-W8.1
        # pattern — the pin moves UP with a mandated new check). The power
        # line reports [PASS] "standard (default)" here: this workspace has
        # no power.json, and doctor must never create one — which U-M5's
        # read-only commands also honor. This workspace never installs the
        # catalog, so all three U-A2 lines stay [PASS] and the warn count
        # is unaffected.
